...
object StepManager {
     ...
     */
    private fun <T : StepImpl> beginExecute(stepImpl: Class<T>, step: Int, delay: Long = Assists.Config.defaultDelay, data: StepData? = null): StepManager {
        isStop = false
     ...
     */
    fun <T : StepImpl> execute(stepImpl: Class<T>, step: Int, delay: Long = Assists.Config.defaultDelay, data: StepData? = null, isBegin: Boolean = false) {
    ...
    fun <T : StepImpl> register(stepImpl: Class<T>) {
        StepCollector(stepImpl.name).let {
            ...
            Log.d(Assists.Config.logTag, "register->${stepImpl.name}")
        }
    }
}
